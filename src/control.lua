...
local uiDefinitions = {
	["PlayerGhostCraftButton"] = {
		type = "sprite-button",
		name = "PlayerGhostCraft",
...
script.on_init(function()
	...
	for _, p in pairs(game.players) do
		if not mod_gui.get_button_flow(p).PlayerGhostCraft then
			local button = mod_gui.get_button_flow(p).add(uiDefinitions["PlayerGhostCraftButton"])
		end
	end
end)
				...
				for _, stack in pairs(ghost.ghost_prototype.items_to_place_this) do
					...
					if player.force.recipes[item] ~= nil and
						...
						if recipe ~= nil and recipe.allow_as_intermediate then
							if player.begin_crafting(
								{count = 1, recipe = stack.name, silent = true}) >
								0 then
...
script.on_event(defines.events.on_gui_click, function(event)
	...
	if name == "PlayerGhostCraft" then autoCraft(player) end
end)

script.on_event(defines.events.on_player_created, function(event)
	local button = mod_gui.get_button_flow(game.players[event.player_index]).add(uiDefinitions["PlayerGhostCraftButton"])
end)